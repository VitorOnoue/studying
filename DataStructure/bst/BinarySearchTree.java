...
public class BinarySearchTree {
  ...
  private int countNodesRec(TreeNode root) {
    ...
    return 1 + countNodesRec(root.getLeft()) + countNodesRec(root.getRight());
  }

  public int countNodesIter() {
    boolean encerraIter = false;
    int iterCont = 0;
    ArrayList<TreeNode> listaNodes = new ArrayList<TreeNode>();
    TreeNode nodePrinc = getRoot();
    if (nodePrinc != null) {
      listaNodes.add(nodePrinc);
      while (!encerraIter) {
        try {
          TreeNode nodeAux = listaNodes.get(iterCont);
          TreeNode left = nodeAux.getLeft();
          TreeNode right = nodeAux.getRight();
          if (left != null) {
            listaNodes.add(left);
          }
          if (right != null) {
            listaNodes.add(right);
          }
          iterCont += 1;
  ...
  public int minValueIter(TreeNode root) {
    if (root == null) {
      return 0;
    }
  ...
  private int minValueRec(TreeNode root) {
    ...
    return minValueRec(root.getLeft());
  }

  public int maxValueIter(TreeNode root) {
    if (root == null) {
      return 0;
    }
  ...
  private int maxValueRec(TreeNode root) {
    ...
    return maxValueRec(root.getRight());
  }
